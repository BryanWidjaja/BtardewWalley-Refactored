...
public class EventCommandRegistry {
    ...
    public Map<GameEvent, Command> build() {
        ...
        commands.put(
            GameEvent.TOOL_STORE,
            () -> views.getToolStore().show(storeViewModel, playerViewModel)
        );
        commands.put(
            GameEvent.ANIMAL_STORE,
            () -> views.getAnimalStore().show(storeViewModel, playerViewModel)
        );
        commands.put(
            GameEvent.FARM_STORE,
            () -> views.getFarmStore().show(storeViewModel, playerViewModel)
        );
        ...
        commands.put(
            GameEvent.PLANT_PROMPT,
            () -> views.getPlant().showPlantPrompt(playerViewModel, mapViewModel)
        );
        commands.put(
            GameEvent.COLLECT_ANIMAL,
            () -> views.getHarvest().showCollectAnimal(playerViewModel, mapViewModel)
        );
